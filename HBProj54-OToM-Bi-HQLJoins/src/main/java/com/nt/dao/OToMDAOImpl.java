...
public class OToMDAOImpl implements OToMDAO {
	...
	public void loadDataUisngChildsAndJoins() {
		...
		query = ses.createQuery(
				"select ph.phone,ph.numberType,u.userId,u.firstName  from PhoneNumber ph full join ph.parent u");
		// execute Query
		list = query.list();
		// process the Result
		for (Object row[] : list) {
	...
	public void loadDataUisngParentsAndJoins() {
		...
		// prepare Query
		query = ses
				.createQuery("select u.userId,u.firstName,ph.phone,ph.numberType  from User u full join u.phones ph");
		// execute Query
		list = query.list();
		// process the Result
		for (Object row[] : list) {
